...
//   static long backup = 0;
   public PureMonte(Board board, long time, double c)
   {
      this.c = c;
      root = new Node(board.clone(), c);
      this.time = time;
   }
   
   public void go(int iterations)
   {
      Crawler worker = new Crawler(root, c);
      Node t;
      double v;
      long start;
...
class Node extends Board
{
   ...
   public int turn;
   public Integer[] move;
   public Node(Node parent, Board board, Integer[] move, double c)
   {
      this.c = c;
   ...
   public static Integer[] deadMove(){
      Integer[] tmp = {-1, -1};
      return tmp;
   }
   
   public Node(Board board, double c)
   {
      this(null, board, deadMove(), c);
   }
}
...
class Crawler{
   public double c;
   public Node root;
   public Crawler(Node root, double c){
      this.c = c;
      this.root = root;
   }
   ...
   public Node Expand(Node cur)
   {
      ...
      Integer[] move = cur.board.moves.get(cur.children.size());

      Board childBoard = cur.board.play_next(move);
      Node child = new Node(cur, childBoard, move, c);
